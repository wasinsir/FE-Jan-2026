...
describe("ProductsRepository", () => {
  ...
  it("rejects creating a product with duplicate SKU", async () => {
    ...
    await expect(
      repo.createProduct({ ...makeProduct(), sku: "DUP-1" }),
    ).rejects.toThrow();
  });
  ...
  it("sellProduct decreases stock and errors when out of stock or not found", async () => {
    ...
    await repo.createProduct(p);
    const sold = await repo.sellProduct(p.id, 1);
    expect(sold.stock).toBe(0);
    await expect(repo.sellProduct(p.id, 1)).rejects.toThrow(/out of stock/i);
    await expect(repo.sellProduct("nope", 1)).rejects.toThrow(/not found/i);
  });
  ...
  it("updateProductPrice and bulkPriceUpdate behave correctly", async () => {
    ...
    const bulk = await repo.bulkPriceUpdate([
      { id: 1, newPrice: 11 },
      { id: 2, newPrice: 12 },
    ]);
